...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # gradient accumulation: scale the loss and step every accum_iter batches
        ...
        metric_logger.update(class_error=loss_dict_reduced.get('class_error', torch.tensor(0.)))
        metric_logger.update(sub_error=loss_dict_reduced.get('sub_error', torch.tensor(0.)))
        metric_logger.update(obj_error=loss_dict_reduced.get('obj_error', torch.tensor(0.)))
        metric_logger.update(rel_error=loss_dict_reduced.get('rel_error', torch.tensor(0.)))
        metric_logger.update(lr=optimizer.param_groups[0]["lr"])

    # flush any remaining accumulated gradients
    if len(data_loader) % accum_iter != 0:
        if max_norm > 0:
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm)
        optimizer.step()
        optimizer.zero_grad()
